# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 11:32:29 2019
...
def getAllProductLink(url):
    url = url.encode('utf-8')
    html = session.get(url,headers = headers, proxies=proxies,verify=False)
    bs = html.text
    skuids = [m.start() for m in re.finditer('skuid=\"', bs)]
    itemids = [m.start() for m in re.finditer('itemid=\"', bs)]
    itemnames = [m.start() for m in re.finditer('itemname=\"', bs)]
    result = []
    for i in range(len(skuids)):
        result.append(r"https://www.vmall.com/product/" + str(int(bs[itemids[i]:itemnames[i]].split("\"")[1])) + ".html#" + str(int(bs[skuids[i]:itemids[i]].split("\"")[1])))
    return result
...
def getProductInfo(url):
    ...
    html = session.get(url,headers = headers, proxies=proxies,verify=False)
    bs = html.text    
    infos = re.findall(r"<label>(\S+)</label><span>(\S+)</span>", bs, re.S)
#    infos1 = re.findall(r"<label>(\S+)</label><span>(\S+)</span>", bs, re.S)
#    for name, value in infos:
#        print(name, " : ", lxml.html.fromstring(name).text)
#        print(value, " : ", lxml.html.fromstring(value).text)
#    if(len(info) > 20):
    return dict((lxml.html.fromstring(name).text, lxml.html.fromstring(value).text) for name, value in infos)
#    else:
        
#汇总
def main():
    #爬取的基准网页（s = 0）
    base_url = 'https://www.vmall.com/search?keyword=%E8%B7%AF%E7%94%B1%E5%99%A8'
    ...
    result = []
    links = getAllProductLink(base_url)
    for link in links:
